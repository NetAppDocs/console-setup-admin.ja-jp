...
---
= NetApp ConsoleにおけるGoogle Cloudのプロジェクトと権限について説明します
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. コンソールからコンソール エージェントを起動する権限を持つ Google アカウントを使用して、コンソール エージェントを展開する必要があります。
. コンソールエージェントをデプロイする際には、エージェント用の https://cloud.google.com/iam/docs/service-accounts["サービスアカウント"^]を選択するように求められます。コンソールは、サービス アカウントから権限を取得して、Cloud Volumes ONTAPシステムの作成と管理、NetAppバックアップとリカバリを使用したバックアップの管理などを行います。権限は、サービス アカウントにカスタム ロールを割り当てることで付与されます。
