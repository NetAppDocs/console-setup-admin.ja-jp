...
---
= NetApp Console エージェントの Azure 権限
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
検索と復元機能を使用するとき、エージェントは次の API 要求を行います。これらの権限は、2025 年 2 月のインデックス v2 のリリース前に従来のインデックス機能を有効にした場合にのみ必要です。
...
.3+| 柔軟なオーケストレーションモードで仮想マシン スケール セットを使用して、Cloud Volumes ONTAPの特定のゾーンを指定します | Microsoft.Compute/仮想マシンスケールセット/書き込み | はい | いいえ | いいえ 
