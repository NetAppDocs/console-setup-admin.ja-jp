...
summary: NetApp Consoleを使用すると、コンソール エージェントの導入時に適切な IAM 認証情報を提供し、それらを AWS Marketplace サブスクリプションに関連付けて単一または複数の AWS アカウントに課金することで、AWS 認証情報とマーケットプレイス サブスクリプションを管理し、 Cloud Volumes ONTAPやその他のデータ サービスを安全かつコスト効率よく導入できます。 
---
...
コンソールからコンソールエージェントをデプロイする場合、IAMロールのARN、またはIAMユーザーのアクセスキーを指定する必要があります。認証方法には、AWSにコンソールエージェントをデプロイするための権限が必要です。必要な権限はlink:task-install-agent-aws-console.html#aws-permissions-agent["AWS のエージェント展開ポリシー"]に記載されています。

ConsoleがAWSでConsole agentを起動すると、agent用のIAMロールとプロファイルが作成されます。また、そのAWSアカウント内のリソースとプロセスを管理する権限をConsole agentに付与するポリシーもアタッチされます。link:reference-permissions-aws.html["エージェントが権限をどのように使用しているかを確認する"]。

image:diagram_permissions_initial_aws.png["コンソールが AWS アカウントにコンソールエージェントを展開する様子を示す概念図。"]
...
初期のAWS認証情報を使用してすべてのCloud Volumes ONTAPシステムを導入することも、認証情報を追加することもできます。
...
image:diagram_permissions_multiple_aws.png["2 つの追加アカウントを示す概念図。それぞれに IAM ポリシーがあります。1 つは IAM ユーザーに接続され、もう 1 つは IAM ロールに接続されます。"]
...
Cloud Volumes ONTAPやその他のNetAppデータ サービスの料金を時間単位 (PAYGO) または年間契約で支払うには、コンソール エージェントに追加する認証情報を AWS Marketplace サブスクリプションに関連付ける必要があります。link:task-adding-aws-accounts.html#subscribe["AWSサブスクリプションを関連付ける方法を学ぶ"]。
...
上記のセクションでは、Consoleエージェントの推奨されるデプロイ方法である、Consoleからのデプロイ方法について説明しています。AWS Marketplaceから AWS にエージェントをデプロイすることもできます。また、Consoleエージェントソフトウェアを独自のLinuxホストまたはvCenterに手動でインストールすることもできます。
