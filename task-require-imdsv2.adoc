...
summary: NetApp Consoleは、コンソール エージェントとCloud Volumes ONTAPを使用して IMDSv2 をサポートします。ほとんどの場合、IMDSv2 は新しい EC2 インスタンスで自動的に構成されます。 IMDSv1 は 2024 年 3 月より前に有効化されました。セキュリティ ポリシーで必要な場合は、EC2 インスタンスで IMDSv2 を手動で設定する必要がある場合があります。 
---
= NetApp ConsoleにおけるAmazon EC2インスタンスでのIMDSv2の使用を必須にする
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
