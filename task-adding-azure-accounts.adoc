...
image:diagram_azure_authentication.png["API 呼び出しを行う前に、コンソールが Microsoft Entra ID から認証と承認を取得する様子を示す概念図。コンソール ロールは、Active Directory 内の権限を定義します。これは、1 つ以上の Azure サブスクリプションと、コンソール アプリケーションを表すサービス プリンシパル オブジェクトに関連付けられています。"]
...
* サブスクリプションを設定するには、まずコンソールエージェントを作成する必要があります。link:concept-agent-install-options.html["コンソールエージェントの作成方法を学ぶ"]。
...
.結果
「詳細と認証情報」ページから、別の認証情報に切り替えることができます https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-deploying-otc-azure.html["コンソールにシステムを追加するとき"^]。
...
. 認証情報を既存のサブスクリプションに関連付けるには、ドロップダウンリストからサブスクリプションを選択し、* Configure *を選択します。
