...
summary: NetApp Console管理の最新情報をご確認ください。 
---
= NetApp Consoleの最新情報
:hardbreaks:
:allow-uri-read: 
...
BlueXPを制限モードまたはプライベートモードで使用する場合、ユーザーインターフェイスは、クラウドリージョンまたはオンプレミスにデプロイされたコネクタ仮想マシンからアクセスできます。デフォルトでは、BlueXPはローカルのWebブラウザから安全なHTTPSアクセスを提供するために、自己署名SSL証明書を使用します。
...
コネクタに関するAWSおよびAzureのポリシーを更新し、不要になった権限を削除しました。これらの権限は、BlueXPエッジキャッシングおよびAzureとAWSにおけるKubernetesクラスターの検出と管理に関連していました。
...
----
これらの権限は、Cloud Volumes ONTAP の仮想マシン スケール セットのサポートに必要です。既存のコネクタがあり、この新機能を使用する場合は、Azure 認証情報に関連付けられているカスタム ポリシーにこれらのアクセス許可を追加する必要があります。
...
RHEL 7 または CentOS 7 上で既にエージェントが実行されている場合は、NetApp はオペレーティングシステムを RHEL 8 または 9 にアップグレードまたは変換することをサポートしていません。サポートされているオペレーティングシステム上に新しいエージェントを作成する必要があります。切り替えを行うには、以下の概要手順に従ってください：
...
** https://docs.netapp.com/us-en/storage-management-fsx-ontap/use/task-create-fsx-system.html["FSx for ONTAPシステムを作成または検出する"^]
