...
Microsoft Entra ID とのフェデレーション接続を設定して、Console のシングル サインオン（SSO）を有効にすることができます。このプロセスでは、Console をサービス プロバイダとして信頼するように Microsoft Entra ID を設定してから、Console で接続を作成します。
...
+
*** *リダイレクト URL* には、以下を使用してください。 `https://services.cloud.netapp.com`
...
. Microsoft Entra IDとの接続を作成します。
+
.. 前の手順で作成したクライアントIDとクライアントシークレットを入力します。
