...
---
= Azure にNetApp Console からコンソール エージェントをデプロイします
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Console から Azure に Console エージェントをデプロイするには、ネットワークの設定、Azure のアクセス許可の準備を行い、その後 Console エージェントをデプロイする必要があります。

Azureにエージェントを導入すると、選択したVNet内でLinuxを実行するVMとConsoleエージェントソフトウェアが起動します。


NOTE: Cloud Volumes ONTAPシステムを管理するコンソールエージェントを同じAzureリージョン、または https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Azure リージョン ペア"^]にデプロイします。これにより、Cloud Volumes ONTAPとそれに関連付けられたストレージアカウント間でAzure Private Link接続が使用されます。
...
. *仮想マシン認証*ページで、Azure権限の設定方法に一致する認証オプションを選択します：
+
...
+
仮想マシンの認証方法は、パスワードまたはSSH公開鍵のいずれかです。

+
+ https://learn.microsoft.com/en-us/azure/virtual-machines/linux-vm-connect?tabs=Linux["Azure の Linux VM への接続について学習します"^]

. *詳細*ページで、エージェントの名前を入力し、タグを指定し、NetApp Consoleが必要な権限を持つ新しいロールを作成するか、またはlink:reference-permissions-azure.html["必要な権限"]で設定済みの既存のロールを選択するかを選択します。
+
...
コンソール エージェントを作成したのと同じ Azure アカウントに Azure Blob ストレージがある場合は、*システム* ページに Azure Blob ストレージが自動的に表示されます。 https://docs.netapp.com/us-en/bluexp-blob-storage/index.html["NetApp Consoleから Azure Blob ストレージを管理する方法を学びます"^]
