...
+
これらの手順では標準モードでのコンソールの使用方法を説明しているため、制限モードは無効にしておく必要があります。安全な環境が整っていて、このアカウントをコンソールのバックエンドサービスから切断したい場合にのみ、制限モードを有効にしてください。もしそうなら、link:./task-quick-start-restricted-mode.html["NetApp Consoleを制限モードで使い始めるための手順に従います"]。
