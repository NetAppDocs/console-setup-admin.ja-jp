...
Google Cloud マシンタイプ:: CPU と RAM の要件を満たすインスタンス タイプ。  NetApp はn2-standard-8 を推奨します。
+
--
...
.ポッドマン
--
Podman をインストールして設定します。

* podman.socket サービスを有効にして起動します
* Python3をインストールする
* それぞれのpodman-composeパッケージをインストールします。
* podman-composeをPATH環境変数に追加する
* PodmanバージョンがCNIではなくNetavark Aardvark DNSを使用していることを確認します。
...
. Podman をインストールします。
+
Red Hat Enterprise Linux リポジトリから Podman を取得します。
...
----
. podman-compose を取得するには EPEL をインストールします。
+
...
+
次の手順に従って、サポートされている Docker エンジン バージョンをインストールします。コンソールは最新バージョンをサポートしていないため、インストールしないでください。
