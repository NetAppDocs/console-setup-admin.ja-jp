...
組織のIDとアクセスを管理するには、*組織管理者*または*スーパー管理者*の権限が必要です。*フォルダーまたはプロジェクト管理者*の権限を持っている場合、自分がアクセス権限を持つフォルダーとプロジェクトのみを管理できます。
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-5.png["五"] 組織の認証（任意）
[role="quick-margin-para"]
チームで組織認証を使用している場合は、Console組織をNetAppビジネスアカウントのIDにリンクしてください。

[role="quick-margin-para"]
link:task-verify-organization.html["組織を確認する方法を学ぶ"]。

.関連情報
* link:concept-identity-and-access-management.html["NetApp Consoleのアイデンティティとアクセス管理について学ぶ"]
* link:concept-verified-organization.html["NetApp Consoleで認証された組織について学ぶ"]
