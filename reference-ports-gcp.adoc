...
summary: エージェントのGoogle Cloudファイアウォールルールには、インバウンドルールとアウトバウンドルールの両方が必要です。ConsoleからConsoleエージェントを作成すると、NetApp Consoleはこのセキュリティグループを自動的に作成します。その他のインストールオプションについては、このセキュリティグループを手動でセットアップする必要があります。 
---
= NetApp Console エージェントファイアウォールルール（Google Cloud）
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
エージェントのGoogle Cloudファイアウォールルールには、インバウンドルールとアウトバウンドルールの両方が必要です。ConsoleからConsoleエージェントを作成すると、NetApp Consoleはこのセキュリティグループを自動的に作成します。その他のインストールオプションについては、このセキュリティグループを手動でセットアップする必要があります。
...
| DNS | UDP | 53 | DNS | データ分類によるDNS解決に使用されます 
|===